...
public class ServiceLane {
    ...
    static int[] serviceLane(int n, int[][] cases, int[] width) {
        ...
        int[] minVehicalLenArray = new int[cases.length];
        int min;
        int roadMinIndex, roadMaxIndex;

        for(int i = 0; i < cases.length; i++) {
            roadMinIndex = cases[i][0];
            roadMaxIndex = cases[i][1];
            min = width[roadMinIndex];

            for(int roadSizeIndex = roadMinIndex + 1; roadSizeIndex <= roadMaxIndex; roadSizeIndex++) {
                if(min > width[roadSizeIndex]) {
                    min = width[roadSizeIndex];
                }
            }

            minVehicalLenArray[i] = min;
        }

        return minVehicalLenArray;
    }
